...
class SiteHelpers
{
    ...
    {	
        ...
        $api_key = config('constants.sms_details.api_key');
        $username = config('constants.sms_details.username');
        $full_mobile = $mobile;
        $api_key = config('constants.sms_details.api_key');
        $username = config('constants.sms_details.username');
        $full_mobile = $mobile;
        $str = 'username=nookesh&message='.$message.'&sendername=DSEREV&smstype=TRANS&numbers='.$mobile.'&apikey=c4d3ef07-49ff-48a1-a585-a40b5a4ec0e2';
       // $str = "sendername=BIEAP OTP&message=$message&numbers=$mobile&api_key=$api_key&username=$username";

        // Generated by curl-to-PHP: http://incarnate.github.io/curl-to-php/
        $ch = curl_init();

        curl_setopt($ch, CURLOPT_URL, 'http://sms.messageindia.in/v2/sendSMS?');
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
        //curl_setopt($ch, CURLOPT_POSTFIELDS, "from=Vonage SMS API&text=Hello from Vonage&to=916375768042&api_key=192d0502&api_secret=0dySQa3utxa67cfU");
